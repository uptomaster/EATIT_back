...
public class IngredientAddController implements Execute{
	
	@Override
	public Result execute(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		System.out.println("===IngredientAddController 진입 성공===");
	    ...
	    // 2. path 변수 초기화
	    	path = "/app/sellerMyPage/ingredientSalesWrite.jsp";  // 기본값 설정
		...
		result.setPath(path);
		result.setPath(path);
		System.out.println("[IngredientAddController] path :  "+path);
		return result;	
		}
