...
public class RecipeBoardReadOkController implements Execute{

	@Override
	public Result execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		System.out.println("====RecipeBoardReadOkController 실행====");
		...
		if(postNumberStr == null || postNumberStr.trim().isEmpty()){
			...
			result.setPath("/community/recipeListOk.co"); //게시글 목록 페이지로 리다이렉트
		...
		//DB에서 게시글 가져오기
		PostDetailDTO postDetailDTO = communityDAO.recipePostSelect(postNumber);
		// 이미지 리스트 가져오기
