        ...
        <div class="store_info_food">
                ...
                <c:forEach var="ingre" items="${ingreList}">
                  <form action="${pageContext.request.contextPath}/sellerMyPage/editFood.se" method="post" class="editFoodForm">
                    <div class="store_info_ingredient_menu_list">
                      <!-- 음식 사진 출력 -->
                      <div class="img-box">
                        <img src="${pageContext.request.contextPath}/upload/${ingre.getItemImageSystemName()}" alt="${ingre.itemName }" />
                      </div>
                      <!-- 사진 옆 정보부분 -->
          ...
          <div class="seller_store_info_pagination">
            <ul class="seller_store_info_pagination_ul">
              <c:if test="${prev}">
                <li>
                  <a href="${pageContext.request.contextPath}/sellerMyPage/storeIngre.se?page=${startPage - 1}" class="prev">&lt;</a>
                </li>
              </c:if>
              ...
              <c:forEach var="i" begin="${realStartPage}" end="${endPage}">
                <c:choose>
                  <c:when test="${!(i == page) }">
                    <li>
                      <a class="pagination_item" href="${pageContext.request.contextPath}/sellerMyPage/storeIngre.se?page=${i}">
                        <c:out value="${i}" />
                      </a>
                    </li>
                  </c:when>
              ...
              <c:if test="${next}">
                <li>
                  <a href="${pageContext.request.contextPath}/sellerMyPage/storeIngre.se?page=${endPage + 1}" class="next">&gt;</a>
                </li>
              </c:if>
            </ul>
          </div> <!-- 페이지네이션 끝 -->
