		...
		<div class="admin_inner">
			...
			<div class="admin_list_title">
				<ul class="admin_list">
					...
					<li class="admin_list_menu"><a
						href="${pageContext.request.contextPath}/admin/suspend/list.ad">정지목록</a>
					</li>
					<li class="admin_list_menu"><a
			...
			<div class="admin_list_whitebox">
				<ul class="admin_list_name">
					...
					<li class="admin_list_row col-user">피신고자</li>
				...
				<ul class="admin_list_valuebox">
					<c:choose>
							...
							<c:forEach var="r" items="${reportList}">
								<li class="admin_list_value">
									...
									<p class="admin_list_row col-reason">
										<span class="tag_reason ${r.postReportReason}">
											${r.postReportReason} </span>
									</p>
									<p class="admin_list_row col-post">${r.postNumber}</p>
									<p class="admin_list_row col-user" title="${r.reportedName}">
										${r.reportedName} (${r.reportedType})</p>
									<p class="admin_list_row col-reporter" title="${r.reporterId}">
										${r.reporterId}</p>
									<p class="admin_list_row col-date">${r.postReportDate}</p>
									<p class="admin_list_row col-action">
									<form
									...
									<form
										...
										method="post" style="display: inline;">
										<input type="hidden" name="memberNumber"
											value="${r.reportedNumber}"> <input type="hidden"
											name="reason" value="${r.postReportReason}">
						...
						<c:otherwise>
							<li class="admin_list_value empty">등록된 신고 내역이 없습니다.</li>
						</c:otherwise>
					</c:choose>
				</ul>
			</div>
			...
			<div class="admin_pagenation_search">
				<ul class="admin_pagenation">
					<c:if test="${prev}">
						<li><a
							href="${pageContext.request.contextPath}/admin/report/list.ad?page=${startPage - 1}">&lt;</a>
						</li>
					</c:if>
					...
					<c:forEach var="i" begin="${startPage}" end="${endPage}">
						<c:choose>
							...
							<c:otherwise>
								<li><a
									href="${pageContext.request.contextPath}/admin/report/list.ad?page=${i}">${i}</a>
								</li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					...
					<c:if test="${next}">
						<li><a
							href="${pageContext.request.contextPath}/admin/report/list.ad?page=${endPage + 1}">&gt;</a>
						</li>
					</c:if>
				</ul>
			</div>
