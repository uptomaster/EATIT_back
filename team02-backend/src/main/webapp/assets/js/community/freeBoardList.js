document.addEventListener("DOMContentLoaded", () => {
    ...
    const promises = filteredRows.map(async row => {
      ...
      try {
        const res = await fetch(`${window.ctx}/community/postlike.co?postNumber=${postNumber}`, {
          method: "GET", // 추천수만 가져오는 GET API 필요
        ...
        if (data?.likeCount !== undefined) {
          likeEl.textContent = data.likeCount; // 숫자만 반영
        }
      } catch (err) {
  ...
  listBody.addEventListener("click", async e => {
    ...
    try {
      ...
      if (!data.success) {
        ...
        likeEl.textContent = data.likeCount; // 숫자 바로 반영
      }
    } catch (err) {
      console.error(err);
      alert("추천 처리에 실패했습니다.");
  ...
  window.addEventListener("pageshow", function(event) {
      if (event.persisted) { // 브라우저 bfcache에서 로드된 경우
